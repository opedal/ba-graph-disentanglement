...
def preprocess_adj_tensor_by_filter(adj_tensor, num_filters=1,symmetric=True):
    if num_filters == 1:
        return preprocess_adj_tensor(adj_tensor=adj_tensor,symmetric=symmetric)
    elif num_filters == 2:
        return preprocess_adj_tensor_with_identity(adj_tensor=adj_tensor,symmetric=symmetric)
    elif num_filters == 3:
        return preprocess_adj_tensor_with_two_identities(adj_tensor=adj_tensor,symmetric=symmetric)
    else:
        print("not implemented for num_filters other than 1,2,3")
...
def preprocess_adj_tensor_with_two_identities(adj_tensor, symmetric=True):
    adj_out_tensor = []
    for i in range(adj_tensor.shape[0]):
        adj = adj_tensor[i]
        adj = adj + np.eye(adj.shape[0])
        adj = normalize_adj_numpy(adj, symmetric)
        adj = np.concatenate([np.eye(adj.shape[0]), np.eye(adj.shape[0]), adj], axis=0)
        adj_out_tensor.append(adj)
    adj_out_tensor = np.array(adj_out_tensor)
    return adj_out_tensor
